...
function deal(hand) {
  ...
  if (hand === cHand) {
    ...
    temp.classList.add("card textCenter");
    ...
    if (compHand.length < 1) {
      ...
      let pic = document.createElement("img");
      pic.src = `images/${cHand[cHand.length - 1].face}${
        cHand[cHand.length - 1].suit
      }.png`;
      temp.append(pic);
    }
  } else {
    ...
    let pic = document.createElement("img");
    pic.src = `images/${pHand[pHand.length - 1].face}${
      pHand[pHand.length - 1].suit
    }.png`;
    temp.append(pic);
  }
